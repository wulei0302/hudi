  ...
  public void runMerge(HoodieTable<T, HoodieData<HoodieRecord<T>>, HoodieData<HoodieKey>, HoodieData<WriteStatus>> table,
    ...
    try {
      ...
      if (baseFile.getBootstrapBaseFile().isPresent()) {
        ...
        if (needToReWriteRecord) {
          readerIterator = new RewriteIterator(reader.getRecordIterator(HoodieAvroIndexedRecord::new), readSchema, reader.getSchema(), table.getConfig().getProps(), renameCols);
        } else {
